(function () {
    ...
    function parse(markdown) {
        ...
        lines.forEach(function (rawLine, index) {
            ...
            var itemMatch = line.match(/^-\s*(\d{2}:\d{2})\s*\|\s*(.+?)\s*$/u);
            if (itemMatch) {
                if (!currentDay) {
                    console.warn("[school-parser] 요일 헤더 없이 일정이 나왔습니다:", line, "line:", index + 1);
                    return;
                }
            ...
            console.warn("[school-parser] 형식이 잘못된 줄을 무시합니다:", line, "line:", index + 1);
        });
